...
`authGuard` é uma função (`CanActivateFn`), não uma classe. Ele reverifica a sessão do Supabase a cada navegação para uma rota protegida, evitando uma race condition onde a sessão ainda não tinha carregado.
